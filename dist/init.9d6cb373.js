...
var ICONS = exports.ICONS = ["fish", "poop", "weather"]; // Array of icon names
var SCENES = exports.SCENES = ["day", "rain"]; // Array of scene names
var TICK_RATE = exports.TICK_RATE = 3000; // The tick rate of the game (in milliseconds)
var RAIN_CHANCE = exports.RAIN_CHANCE = 0.2; // The chance of rain (as a decimal)
var DAY_LENGTH = exports.DAY_LENGTH = 60; // The length of a day (in seconds)
var NIGHT_LENGTH = exports.NIGHT_LENGTH = 5; // The length of a night (in seconds)

// Functions to get the next hunger, die, and poop times
var getNextHungerTime = exports.getNextHungerTime = function getNextHungerTime(clock) {
  return Math.floor(Math.random() * 3) + 8 + clock;
}; // Returns a random time for the next hunger event
var getNextDieTime = exports.getNextDieTime = function getNextDieTime(clock) {
  return Math.floor(Math.random() * 3) + 3 + clock;
}; // Returns a random time for the next die event
var getNextPoopTime = exports.getNextPoopTime = function getNextPoopTime(clock) {
  return Math.floor(Math.random() * 3) + 8 + clock;
}; // Returns a random time for the next poop event
...
var modFox = exports.modFox = function modFox(state) {
  document.querySelector(".fox").className = "fox fox-".concat(state);
};
var modScene = exports.modScene = function modScene(state) {
  document.querySelector(".game").className = "game ".concat(state);
};
var togglePoopBag = exports.togglePoopBag = function togglePoopBag(show) {
  document.querySelector(".poop-bag").classList.toggle("hidden", !show);
};
var writeModal = exports.writeModal = function writeModal() {
  var text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "";
  document.querySelector(".modal").innerHTML = "<div class=\"modal-inner\">".concat(text, "</div>");
};
...
var gameState = {
  ...
  // Start celebrating
  startCelebrating: function startCelebrating() {
  ...
  // End celebrating
  endCelebrating: function endCelebrating() {
  ...
  determineFoxState: function determineFoxState() {
  ...
  clearTimes: function clearTimes() {
    this.wakeTime = -1;
    this.sleepTime = -1;
    this.hungryTime = -1;
    this.dieTime = -1;
...
var handleUserAction = exports.handleUserAction = gameState.handleUserAction.bind(gameState);
...
var _constants = require("./constants");
// Importing the ICONS constant from the constants.js file
// Function to toggle the "highlighted" class on an icon element
var toggleHighlighted = function toggleHighlighted(icon, show) {
  return document.querySelector(".".concat(_constants.ICONS[icon], "-icon")).classList.toggle("highlighted", show);
};

// Function to initialize the buttons and handle user actions
function initButtons(handleUserAction) {
  var selectedIcon = 0; // Initializing the selectedIcon variable to 0
  ...
  function buttonClick(_ref) {
    ...
    if (target.classList.contains("left-btn")) {
      ...
      selectedIcon = (2 + selectedIcon) % _constants.ICONS.length; // Update the selectedIcon variable to the left icon
      toggleHighlighted(selectedIcon, true); // Add the "highlighted" class to the new icon
    } else if (target.classList.contains("right-btn")) {
      // If the right button is clicked
      toggleHighlighted(selectedIcon, false); // Remove the "highlighted" class from the current icon
      selectedIcon = (1 + selectedIcon) % _constants.ICONS.length; // Update the selectedIcon variable to the right icon
      toggleHighlighted(selectedIcon, true); // Add the "highlighted" class to the new icon
    } else {
      // If any other button is clicked
      handleUserAction(_constants.ICONS[selectedIcon]); // Call the handleUserAction function with the selected icon
    }
  }
...
function _asyncToGenerator(fn) { return function () { var self = this, args = arguments; return new Promise(function (resolve, reject) { var gen = fn.apply(self, args); function _next(value) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "next", value); } function _throw(err) { asyncGeneratorStep(gen, resolve, reject, _next, _throw, "throw", err); } _next(undefined); }); }; } // Import the game state object and the handleUserAction function
...
function _init() {
  _init = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
    var nextTimeToTick, nextAnimationFrame;
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          nextAnimationFrame = function _nextAnimationFrame() {
          ...
          nextTimeToTick = Date.now(); // Define the nextAnimationFrame function
